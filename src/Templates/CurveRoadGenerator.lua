--!optimize 2
--!native
-- Packaged copy of the ProceduralCarts CurveRoad generator, used by
...
local ROAD_MATERIAL_VARIANT = "RoadSurface"
...
--[[
	Width is derived from the lane layout rather than being an input:

	Width = (numLanes * laneWidth) + (sidewalkWidth * 2)
...
local Generator: GeneratorModuleDefinition<typeof(defaultAttributes)> = {
	...
	OnGenerate = function(parameters, targetContainer)
		...
		-- Blue adjustments apply to the entry end of the road (u = 0), red to the exit
		...
		-- End poses: entry runs +Z and exit runs +X at zero turns. AdjustBlueDir and
		-- AdjustRedDir yaw each end about the vertical axis through its end-edge centre
		...
		local p1x, p1z = halfX, halfZ - halfWidth
		local t1x, t1z = rot2(1, 0, dirEnd)

		-- Fillet construction: intersect the entry ray and the reversed exit ray at a
		-- corner K, then join them with the largest arc tangent to both; the length
		-- left over on each ray becomes a straight run (at least one of them is zero,
		-- matching the old quarter-circle + straights construction at zero turns).
		local vx, vz = p1x - p0x, p1z - p0z
		local denom = t0x * t1z - t0z * t1x -- cross(T0, T1); negative = clockwise turn
		local rayA = (vx * t1z - vz * t1x) / denom
		local rayB = -(vx * t0z - vz * t0x) / denom
		local sweepAngle = math.acos(math.clamp(t0x * t1x + t0z * t1z, -1, 1))
		local side = if denom < 0 then 1 else -1
		local tanHalf = math.max(math.tan(sweepAngle / 2), 1e-4)
		local filletT = math.max(math.min(rayA, rayB), 0.01)
		local r = filletT / tanHalf
		local lEntry = math.max(rayA - filletT, 0)
		local lExit = math.max(rayB - filletT, 0)
		local arcLen = r * sweepAngle
		local totalLen = lEntry + arcLen + lExit

		-- arc start point, arc centre, and exit tangent point
		local ax, az = p0x + t0x * lEntry, p0z + t0z * lEntry
		local ox, oz = ax + t0z * side * r, az - t0x * side * r
		local bx, bz = p1x - t1x * lExit, p1z - t1z * lExit

		local climbSign = if attributes.Flip then -1 else 1
		local climbBase = if attributes.Flip then size.Y else 0
		-- Cubic Hermite vertical profile: anchored exactly to the bounding box bottom at
		-- the start and top at the end (mirrored by Flip, grades included), with end
		-- slopes set by AdjustBlueGrade/AdjustRedGrade (the grade basis functions u(u-1)^2 and
		-- u^2(u-1) vanish at both ends, so the anchors hold for any grade). Zero grades
		-- reduce this to the old smoothstep, flat at both ends.
		local gradeStartM = climbSign * gradeStartSlope * totalLen
		local gradeEndM = climbSign * gradeEndSlope * totalLen
		local function climbY(u)
			return bottom + climbBase
				+ climbSign * size.Y * (u * u * (3 - 2 * u))
				+ gradeStartM * (u * (u - 1) * (u - 1))
				+ gradeEndM * (u * u * (u - 1))
		end
		local function climbDeriv(u)
			return (climbSign * size.Y * (6 * u * (1 - u))
		...
		local function pathAt(s)
			if s <= lEntry then
				return p0x + t0x * s, p0z + t0z * s, t0x, t0z
			elseif s <= lEntry + arcLen then
				local phi = side * (s - lEntry) / r
				local rx, rz = rot2(ax - ox, az - oz, phi)
				local fx, fz = rot2(t0x, t0z, phi)
				return ox + rx, oz + rz, fx, fz
			end
			local s2 = s - lEntry - arcLen
			return bx + t1x * s2, bz + t1z * s2, t1x, t1z
		end
		-- Bank rolls the cross-section about the (pitched) tangent — i.e. it applies
		-- after dir and grade — blending from AdjustBlueBank (entry) to AdjustRedBank (exit) with a
		...
		for i = 0, innerSliceCount do
			local s = if fullCost > 0 then sForCost(fullCost * (0.5 + i)) else (i + 0.5) / (innerSliceCount + 1) * totalLen

			sliceCFrames[i + 2] = pathSliceCFrameAt(s)
		end
		sliceCFrames[totalNumSegments + 1] = pathSliceCFrameAt(totalLen)
		-- ends need no squaring: the end slices use the exact turned end headings, with
		-- pitch exactly equal to the configured end grade (zero by default)
		...
		local function tri(pa, pb, pc, color, material, mVariant, thickness)
			...
			if crossV.Magnitude < 1e-3 * ab.Magnitude * ac.Magnitude or crossV.Magnitude < 1e-6 then
				return
			end
		...
		if attributes.Blend then
			...
			for _, sgn in { -1, 1 } do
				...
				for i = 1, totalNumSegments do
					...
					-- across a segment (e.g. the straight/arc junctions) the two slice slopes
					-- don't tilt symmetrically fore/aft, so the raw average has an along-track
					-- component that would drift the box off its joints; anchoring to the top
					-- edge and removing that component keeps the boxes registered exactly.
		...
		-- face (where the edge corners sit at zero turn) — blue at the entry end, red
		-- at the exit end, matching the AdjustBlue*/AdjustRed* parameters. The cubes
		...
		if attributes.AdjustShowSnappingHelper then
			...
			helperCube(blue, p0x + halfWidth, climbY(0), -halfZ + 2)
			helperCube(red, halfX - 2, climbY(1), p1z - halfWidth)
			helperCube(red, halfX - 2, climbY(1), p1z + halfWidth)
		end
	end,
}
